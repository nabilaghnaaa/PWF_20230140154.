    ...
    <div class="relative min-h-screen bg-[#020617] py-12 px-4 sm:px-6 lg:px-8 overflow-hidden"> {{-- Main container dengan background dan padding --}}
        ...
        <div class="relative max-w-2xl mx-auto z-10"> {{-- Container utama dengan max width dan centered --}}
            ...
            <div class="flex items-center gap-6 mb-10"> {{-- Header section dengan back button dan title --}}
                ...
                <div> {{-- Title container --}}
                    <h1 class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-rose-400 to-amber-400 tracking-tighter">
                        Create Category {{-- Main title --}}
                    </h1>
                    <p class="text-slate-500 text-sm mt-1 font-medium">Define a new classification for your assets</p> {{-- Subtitle/description --}}
                </div>
            </div>
            ...
            <div class="bg-white/[0.02] backdrop-blur-2xl border border-white/[0.05] rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden"> {{-- Form card container --}}
                ...
                <div class="absolute top-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-fuchsia-500/50 to-transparent"></div> {{-- Decorative top line --}}

                <form action="/category" method="POST" class="space-y-8"> {{-- Form yang submit ke /category route dengan POST method --}}
                        ...
                        <div class="relative"> {{-- Relative container untuk icon positioning --}}
                            <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-slate-500 group-focus-within:text-fuchsia-400 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 11h.01M7 15h.01M13 7h.01M13 11h.01M13 15h.01M17 7h.01M17 11h.01M17 15h.01" /> {{-- Grid icon untuk category --}}
                                </svg>
                            </div>
                            <input type="text" id="name" name="name" value="{{ old('name') }}" {{-- Input field untuk category name dengan old() untuk error handling --}}
                    ...
                    {{-- INFO BOX --}}
                    <div class="p-4 rounded-2xl bg-fuchsia-500/5 border border-fuchsia-500/10"> {{-- Info box untuk tips --}}
                        <p class="text-slate-500 text-[11px] leading-relaxed">
                            <span class="text-fuchsia-400 font-bold">Pro Tip:</span> Categories help in organizing your inventory reports. Use clear, broad terms for better data grouping. {{-- Info message --}}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-6 pt-6 border-t border-white/[0.05]"> {{-- Action buttons container --}}
                        ...
                        <button type="submit" {{-- Submit button untuk save category --}}
                            ...
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /> {{-- Checkmark icon untuk confirm --}}
                            </svg>
                            SAVE CATEGORY {{-- Submit button text --}}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="mt-10 flex flex-col items-center gap-2"> {{-- Footer section dengan decorative text --}}
                <div class="w-12 h-[1px] bg-fuchsia-500/30"></div> {{-- Decorative line --}}
                <p class="text-[10px] font-black uppercase tracking-[0.5em] text-slate-700">
                    Database Protocol v2.4 // Classification Layer {{-- Footer text --}}
                </p>
            </div>
        </div>
    </div>
